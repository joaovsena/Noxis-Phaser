...
export const CURRENCY_LABELS: Record<CurrencyName, string> = {
    copper: 'cobre',
    silver: 'prata',
    gold: 'ouro',
    diamond: 'diamante'
...
export function normalizeWallet(input: Partial<Wallet> | null | undefined): Wallet {
...
export function walletToCopper(wallet: Wallet): number {
    return (
        Math.max(0, Math.floor(Number(wallet.copper || 0)))
...
export function walletFromCopper(totalCopper: number): Wallet {
...
export function toCopperByCurrency(amount: number, currency: CurrencyName): number {
    const safeAmount = Math.max(0, Math.floor(Number(amount || 0)));
    if (currency === 'diamond') return safeAmount * DIAMOND_IN_COPPER;
    if (currency === 'gold') return safeAmount * GOLD_IN_COPPER;
    if (currency === 'silver') return safeAmount * SILVER_IN_COPPER;
    return safeAmount;
}

export function parseCurrencyName(raw: string): CurrencyName | null {
    const key = String(raw || '').trim().toLowerCase();
    if (key === 'cobre' || key === 'copper') return 'copper';
    if (key === 'prata' || key === 'silver') return 'silver';
    if (key === 'ouro' || key === 'gold') return 'gold';
    if (key === 'diamante' || key === 'diamond') return 'diamond';
    return null;
}
