...
type CastContext = {
    ...
    skill: SkillDef;
    skillLevel: number;
    castMode: NonNullable<SkillDef['castMode']>;
...
export class SkillService {
    ...
    constructor(
        ...
        private readonly raiseDead: RaiseDeadFn,
        private readonly commandDead: CommandDeadFn,
        private readonly legionCall: LegionCallFn,
        private readonly armyOfShadows: ArmyOfShadowsFn,
        private readonly getActiveSummonCount: GetActiveSummonCountFn
    ) {}
    ...
    handleSkillCast(player: PlayerRuntime, msg: any) {
        ...
        if (skill.id === 'nec_grave_raise_dead' && !this.raiseDead(player, skillLevel, now)) return;
        ...
        let totalDamage = 0;
        let directPrimaryTarget: any = null;
        ...
        for (const mob of targets) {
            ...
            if (!directPrimaryTarget) directPrimaryTarget = mob;
        }
    ...
    private resolveCastContext(
        ...
        if (castMode === 'direct') {
            ...
            const edgeDistance = currentDistance - (Number(targetMob.size || 0) / 2 + PLAYER_HALF_SIZE);
        ...
        if (!targetPoint) {
            player.pendingSkillCast = null;
            if (!isAutoRetry) this.sendRaw(player.ws, { type: 'system_message', text: 'Escolha um ponto para usar a habilidade.' });
            return null;
        }

        if (distance(player, targetPoint as any) > range) {
            player.pendingSkillCast = {
                skillId,
    ...
    private resolveTargetsForCast(player: PlayerRuntime, context: CastContext) {
        ...
        if (castMode === 'direct') return targetMob ? [targetMob] : [];
        if (castMode === 'self_aoe' || castMode === 'ground') {
            ...
            return allMobs.filter((mob) => distance(center as any, mob) <= radius);
        }
        if (castMode === 'line') {
        ...
        if (castMode === 'cone') {
            ...
            return allMobs.filter((mob) => {
                ...
                if (mobDistance > coneLength) return false;
    ...
    private distanceToSegment(point: any, start: { x: number; y: number }, end: { x: number; y: number }) {
        const px = Number(point?.x || 0);
        const py = Number(point?.y || 0);
        const dx = end.x - start.x;
        const dy = end.y - start.y;
        const lenSq = dx * dx + dy * dy;
        if (lenSq <= 0.0001) return Math.hypot(px - start.x, py - start.y);
