...
export class QuestService {
    ...
    private resolveQuestRewardTemplateId(player: PlayerRuntime, reward: QuestRewardItemDef) {
        ...
        const rarity = String(reward?.rarity || 'branco') as 'branco' | 'verde' | 'azul' | 'roxo' | 'laranja';
        const quality = String(reward?.quality || 'normal') as 'pobre' | 'normal' | 'bom' | 'otimo' | 'excelente';
        if (reward?.classWeapon) {
            const candidate = resolveClassWeaponTemplateId(classId, tier, rarity, quality);
            return BUILTIN_ITEM_TEMPLATE_BY_ID[candidate] ? candidate : '';
        }
        const slot = String(reward?.classEquipmentSlot || '').trim();
        if (!slot) return '';
        const candidate = resolveClassEquipmentTemplateId(classId, slot, tier, rarity, quality);
        return BUILTIN_ITEM_TEMPLATE_BY_ID[candidate] ? candidate : '';
    }
