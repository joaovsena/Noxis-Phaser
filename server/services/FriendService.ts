...
export class FriendService {
    private readonly friendLinks: Map<number, Set<number>> = new Map();
    private readonly friendRequests: Map<string, FriendRequestItem> = new Map();
    private readonly friendRequestWindow: Map<number, number[]> = new Map();
    ...
    async hydrateFriendStateForPlayer(player: PlayerRuntime) {
    ...
    sendFriendState(player: PlayerRuntime) {
    ...
    async pruneExpiredFriendRequests(now: number) {
    ...
    clearFriendRequestsForPlayer(playerId: number) {
        for (const [requestId, request] of this.friendRequests.entries()) {
