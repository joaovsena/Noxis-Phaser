...
export class SkillEffectsService {
    ...
    pruneExpiredSkillEffects(player: PlayerRuntime, now: number = Date.now()) {
    ...
    hasActiveSkillEffect(player: PlayerRuntime, effectId: string, now: number = Date.now()) {
    ...
    removeSkillEffectById(player: PlayerRuntime, effectId: string) {
    ...
    getActiveSkillEffectAggregate(player: PlayerRuntime, now: number = Date.now()) {
        this.pruneExpiredSkillEffects(player, now);
        const out = {
            attackMul: 1,
            defenseMul: 1,
    ...
    applyTimedSkillEffect(player: PlayerRuntime, buff: any, now: number = Date.now()) {
    ...
    applyOnHitSkillEffects(player: PlayerRuntime, dealtDamage: number, now: number = Date.now()) {
    ...
    sendSkillEffect(mapKey: string, mapId: string, payload: any) {
    ...
    broadcastMobHit(player: PlayerRuntime, mob: any) {
        for (const receiver of this.players.values()) {
