...
class GameController {
    constructor(persistence, mobService, lockService) {
        ...
        this.questService = new QuestService_1.QuestService(this.sendRaw.bind(this), this.persistPlayer.bind(this), this.persistPlayerCritical.bind(this), this.grantXp.bind(this), this.grantRewardItem.bind(this), this.grantCurrency.bind(this), this.projectToWalkable.bind(this), (player, npcId) => this.dungeonService?.getNpcUiStateForPlayer(player, npcId) || null);
    ...
    async handleCharacterEnter(ws, msg) {
        try {
            ...
            (0, logger_1.logEvent)('INFO', 'character_enter_send', { order: 3, type: 'quest.state', playerId: player.id });
    ...
    serializeSkillCooldowns(player, now = Date.now()) {
        ...
        for (const [skillId, endsAt] of Object.entries(raw)) {
            ...
            if (!Number.isFinite(safeEndsAt) || safeEndsAt <= now)
                continue;
            out[String(skillId)] = safeEndsAt;
        }
        return out;
    }
    serializeActiveSkillEffects(player, now = Date.now()) {
    ...
    computePublicPlayerSignature(player) {
        ...
        const effectsSignature = Array.isArray(player.activeSkillEffects)
            ? player.activeSkillEffects
                .map((fx) => `${String(fx?.id || '')}:${Number(fx?.expiresAt || 0)}`)
                .sort()
                .join('|')
            : '';
        return [
    ...
    sendRaw(ws, payload) {
        try {
            ...
            (0, logger_1.logEvent)('ERROR', 'send_raw_failed', {
                type: String(payload?.type || 'unknown'),
                error: String(error)
            });
        }
    }
    sendDebugStep(ws, step) {
        try {
            ws?.send(JSON.stringify({
                type: 'debug.step',
                step: String(step || ''),
            ...
            (0, logger_1.logEvent)('ERROR', 'send_debug_step_failed', {
                step: String(step || ''),
                error: String(error)
            });
        }
    }
    sendDebugPacket(ws, order, packetType) {
        try {
            ws?.send(JSON.stringify({
                type: 'debug.packet',
            ...
            (0, logger_1.logEvent)('ERROR', 'send_debug_packet_failed', {
                order: Number(order || 0),
                packetType: String(packetType || ''),
                error: String(error)
            });
